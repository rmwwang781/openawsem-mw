...
def membrane_term(oa, k=1*kilocalorie_per_mole, k_m=20, z_m=1.5, membrane_center=0*angstrom, forceGroup=24, periodic=False):
    ...
    box_size = 50
    # Add Periodic Boundary Condition, which could be edited with "periodicdistance(x, y, z, x0, y0, z0)". 02082024 Rebekah Added --- Start
    if periodic:
        # membrane = CustomExternalForce (f"k_membrane*\
        #         (0.5*tanh({k_m}*((z-{membrane_center})+{z_m}))+0.5*tanh({k_m}*({z_m}-(z-{membrane_center}))))*hydrophobicityScale; r=periodicdistance(0,0,0,0,0,50)")
        # membrane = CustomExternalForce(f"k_membrane * hydrophobicityScale * ( 0.5*tanh({k_m}*(z_periodic + {z_m})) + 0.5*tanh({k_m}*({z_m} - z_periodic)) ); z_periodic=dz - floor(dz / 50) * 50 ;dz=z-{membrane_center}")
        # membrane = CustomExternalForce(f"( 0.5*tanh({k_m}*(z_periodic + {z_m})) + 0.5*tanh({k_m}*({z_m} - z_periodic)) );z_periodic=periodicdistance(0,0,z,0,0,{membrane_center})")
        # membrane = CustomExternalForce(f"( 0.5*tanh({k_m}*(z_periodic + {z_m})) + 0.5*tanh({k_m}*({z_m} - z_periodic)) );z_periodic=z-floor(z/{box_size}+0.5)*{box_size};dz=z-z0")
        membrane = CustomExternalForce(f"k_membrane * hydrophobicityScale *"
                                       f"( 0.5*tanh({k_m}*(z_periodic + {z_m})) + 0.5*tanh({k_m}*({z_m} - z_periodic)) )"
                                    #    f";z_periodic=abs(z-{membrane_center})")
                                       f";z_periodic=periodicdistance(0,0,z,0,0,{membrane_center})")
        membrane.setForceGroup(25)
    else:
        membrane = CustomExternalForce (f"k_membrane*\
    ...
    # if periodic:
    is_periodic=membrane.usesPeriodicBoundaryConditions()
    print("\nmembrane_term is in PBC",is_periodic)     
