...
def vesicle_contact_term(oa, k_contact=4.184, vesicel=True, k_m=20, vesicle_radius=5, vesicle_thickness=3, vesicle_center=(0, 0, 0), k_relative_mem=1.0, periodic=False, parametersLocation=None, burialPartOn=True, withExclusion=True, forceGroup=22,
                gammaName="gamma.dat", burialGammaName="burial_gamma.dat", membraneGammaName="membrane_gamma.dat", r_min=0.45,min_sequence_separation=10 ):
    ...
    if vesicel:
        ...
        contact.addComputedValue("alphaMembrane", 
            f"(0.5*(1 - tanh({eta_switching}*(sqrt((x-{xc})^2 + (y-{yc})^2 + (z-{zc})^2) - {radius_outer}))) * \
            0.5*(1 + tanh({eta_switching}*(sqrt((x-{xc})^2 + (y-{yc})^2 + (z-{zc})^2) - {radius_inner}))))",
            CustomGBForce.SingleParticle)
        # mediated and direct term (write separately may lead to bug)
        contact.addEnergyTerm(f"isCb1*isCb2*((1-alphaMembrane1*alphaMembrane2)*water_part+alphaMembrane1*alphaMembrane2*membrane_part);\
                                water_part=-res_table(0, resId1, resId2)*{k_contact}*\
...
def test_contact_term(oa, k_contact=4.184, vesicel=True, vesicle_radius=5, vesicle_thickness=3, vesicle_center=(0, 0, 0), k_relative_mem=1.0, periodic=False, parametersLocation=None, burialPartOn=True, withExclusion=True, forceGroup=22,
                gammaName="gamma.dat", burialGammaName="burial_gamma.dat", membraneGammaName="membrane_gamma.dat", r_min=0.45,min_sequence_separation=10 ):
    if parametersLocation is None:
        parametersLocation=openawsem.data_path.parameters
    if isinstance(k_contact, float) or isinstance(k_contact, int):
