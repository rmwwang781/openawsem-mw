...
def contact_term(oa, k_contact=4.184, z_dependent=False, z_m=1.5, inMembrane=False, membrane_center=0*angstrom, k_relative_mem=1.0, periodic=False, parametersLocation=None, burialPartOn=True, withExclusion=True, forceGroup=22,
                gammaName="gamma.dat", burialGammaName="burial_gamma.dat", membraneGammaName="membrane_gamma.dat", r_min=0.45,min_sequence_separation=10 ):
    ...
    eta_sigma = 7.0
    rho_0 = 2.6
    min_sequence_separation = min_sequence_separation  # means j-i > 9
...
'''
def pos_contact_term(oa, k_contact=4.184, z_dependent=False, z_m=1.5,k_m=10,k_s=10,k_h=1.5,z_c=1.2, R=3, inMembrane=False, membrane_center=0*angstrom, k_relative_mem=1.0, periodic=False, parametersLocation=None, burialPartOn=True, withExclusion=True, forceGroup=22,
                gammaName="gamma.dat", burialGammaName="burial_gamma.dat", membraneGammaName="membrane_gamma.dat", r_min=0.45,min_sequence_separation=10 ):
    if parametersLocation is None:
        parametersLocation=openawsem.data_path.parameters
    if isinstance(k_contact, float) or isinstance(k_contact, int):
    ...
    if z_dependent:
        # contact.addComputedValue("alphaMembrane", f"0.5*tanh({eta_switching}*((z-{membrane_center})+{z_m}))+0.5*tanh({eta_switching}*({z_m}-(z-{membrane_center})))", CustomGBForce.SingleParticle)
        contact.addComputedValue(
            "S", 
            f"0.5 * (1 + tanh({k_s} * (z - {z_m})))", 
            CustomGBForce.SingleParticle
        )
        contact.addComputedValue(
            "alphaFlat", 
            f"0.5 * (1 + tanh({k_m} * ((z - {membrane_center}) + {z_m}))) + \
            0.5 * (1 + tanh({k_m} * ({z_m} - (z - {membrane_center}))))", 
            CustomGBForce.SingleParticle
        )

        contact.addComputedValue(
            "H_smooth", 
            f"0.5 * (1 - tanh({k_h} * (x^2 + y^2 + (z - {z_c})^2 - {R}^2)))", 
            CustomGBForce.SingleParticle
        )
        contact.addComputedValue(
            "alphaCurved", 
            f"H_smooth * (0.5 * (1 + tanh({k_m} * {z_m})) + 0.5 * (1 + tanh({k_m} * (-{z_m})))) + \
            (1 - H_smooth) * alphaFlat", 
            CustomGBForce.SingleParticle
        )
        contact.addComputedValue(
            "alphaMembrane",
            f"(1 - S) * alphaFlat + S * alphaCurved;",
            CustomGBForce.SingleParticle
        )
        # mediated and direct term (write separately may lead to bug)
        contact.addEnergyTerm(f"isCb1*isCb2*((1-alphaMembrane1*alphaMembrane2)*water_part+alphaMembrane1*alphaMembrane2*membrane_part);\
...
def vesicle_contact_term(oa, k_contact=4.184, vesicel=True, k_m=20, vesicle_radius=5, vesicle_thickness=3, vesicle_center=(0, 0, 0), inMembrane=False, k_relative_mem=1.0, periodic=False, parametersLocation=None, burialPartOn=True, withExclusion=True, forceGroup=22,
                gammaName="gamma.dat", burialGammaName="burial_gamma.dat", membraneGammaName="membrane_gamma.dat", r_min=0.45,min_sequence_separation=10 ):
    # 03192025 Rebekah, apply vesicle function
    if parametersLocation is None:
        parametersLocation=openawsem.data_path.parameters
    if isinstance(k_contact, float) or isinstance(k_contact, int):
        k_contact = k_contact * oa.k_awsem   # just for backward comptable
...
def contact_pore_term(oa, k_contact=4.184, z_dependent=True, z_m=1.5, inMembrane=False, membrane_center=0*angstrom, k_relative_mem=1.0, periodic=False, parametersLocation=".", burialPartOn=True, withExclusion=True, forceGroup=22,
                      gammaName="gamma.dat", burialGammaName="burial_gamma.dat", membraneGammaName="membrane_gamma.dat", r_min=0.45, pore_center_x=0, pore_center_y=0, pore_radius=10, k_pore=0.1):
    if isinstance(k_contact, float) or isinstance(k_contact, int):
        k_contact = k_contact * oa.k_awsem   # just for backward comptable
    elif isinstance(k_contact, Quantity):
        k_contact = k_contact.value_in_unit(kilojoule_per_mole)   # convert to kilojoule_per_mole, openMM default uses kilojoule_per_mole as energy.
    ...
    # Convert pore center to nanometers if they are provided as Quantity objects
    if isinstance(pore_center_x, Quantity):
        pore_center_x = pore_center_x.value_in_unit(nanometer)
    if isinstance(pore_center_y, Quantity):
        pore_center_y = pore_center_y.value_in_unit(nanometer)

    # Convert pore radius to nanometers if it is provided as a Quantity object
    if isinstance(pore_radius, Quantity):
        pore_radius = pore_radius.value_in_unit(nanometer)

    contact.addComputedValue("rho", f"isCb1*isCb2*step(abs(resId1-resId2)-2)*0.25*(1+tanh({eta}*(r-{r_min})))*(1+tanh({eta}*({r_max}-r)))", CustomGBForce.ParticlePair)
    contact.addComputedValue("alphaMembrane", f"(0.5-0.5*tanh({eta_switching}*({pore_radius} - sqrt((x-{pore_center_x})^2 + (y-{pore_center_y})^2)))) *\
                              (0.5*tanh({eta_switching}*((z-{membrane_center})+{z_m}))+0.5*tanh({eta_switching}*({z_m}-(z-{membrane_center}))))", CustomGBForce.SingleParticle)
    ...
    # Modify the energy term to account for the pore
    # This assumes that the original function has a variable `energyTerm` that contains the energy expression
    energyTerm = f"""
    isCb1*isCb2*((1-alphaMembrane1*alphaMembrane2)*water_part +  alphaMembrane1*alphaMembrane2*membrane_part);
    water_part=-res_table(0, resId1, resId2)*{k_contact}*(
        gamma_ijm(0, resName1, resName2)*theta + 
        thetaII*(sigma_water*water_gamma_ijm(0, resName1, resName2) + 
        sigma_protein*protein_gamma_ijm(0, resName1, resName2))
    );
    membrane_part=-res_table(1, resId1, resId2)*{k_contact}*(
        gamma_ijm(1, resName1, resName2)*theta + 
        thetaII*(sigma_water*water_gamma_ijm(1, resName1, resName2) + 
        sigma_protein*protein_gamma_ijm(1, resName1, resName2))
    );
    sigma_protein=1-sigma_water;
    theta=0.25*(1+tanh({eta}*(r-{r_min})))*(1+tanh({eta}*({r_max}-r)));
    ...
    # Add the modified energy term to the force
    contact.addEnergyTerm(energyTerm, CustomGBForce.ParticlePair)
