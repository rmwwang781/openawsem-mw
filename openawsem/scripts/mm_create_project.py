...
def parse_arguments():
    ...
    frag_parser.add_argument("--frag_fragmentLength", type=int, default=10, help="SLength of the fragments to be generated.")
...
class AWSEMSimulationProject:
    ...
    def generate_ssweight_from_fasta(self):
        ...
        predict_property_path = os.getenv("Predict_Property")
        # self.run_command(["$Predict_Property/Predict_Property.sh", "-i", f"{self.name}.fasta"])
        self.run_command([f"{predict_property_path}/Predict_Property.sh", "-i", f"{self.name}.fasta"])
    ...
    def generate_fragment_memory(self,database = "cullpdb_pc80_res3.0_R1.0_d160504_chains29712",fasta = None,N_mem = 20,brain_damage = 1.0,fragmentLength = 9):
        ...
        openawsem.helperFunctions.replace(f"frags.mem", f"{__location__}/data/Gros/", "./fraglib/")
